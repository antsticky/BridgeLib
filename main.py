...
class Board:
    ...
    def create_by_bd_nb(cls, bd_nb):
        ...
        assert len(boards) > bd_nb, "Less board were played"

        return cls(boards[bd_nb])
...
if __name__ == "__main__":
    Galim = Team.create_by_name("GALIM")
    Visoft = Team.create_by_name("VISOFT")
    Gal_Peter = Player.create_by_name("Gál Péter")

    board_32 = Board.create_by_bd_nb(30) # board misindexing
    freq_32 = Freqi.create_by_team_id(Galim.registrationId, 31)

    # They played against each other
    watch32_where_galim_NS = Watch.create_by_team_id(Galim.registrationId, 31)
    watch32_where_visoft_NS = Watch.create_by_team_id(Visoft.registrationId, 31)


    #watch.show_bidding()
    for i in range(32):
        print(f"\n\n------------- board {i+1} -------------")
        watch = Watch.create_by_team_id(Galim.registrationId, i+1)
        watch.show_play(show_play_card=False)
