...
def PrintFut(title, fut, is_print=True):
    if is_print:
        print("{}\n".format(title))
    ...
    for i in range(fut.contents.cards):
        ...
        suit = hands.dcardSuit[fut.contents.suit[i]]
        card = hands.dcardRank[fut.contents.rank[i]]
...
def PrintPar(par):
    print("NS score: {}".format(
        par.contents.parScore[0].value.decode('utf-8')))
    print("EW score: {}".format(
        par.contents.parScore[1].value.decode('utf-8')))
    print("NS list : {}".format(
        par.contents.parContractsString[0].value.decode('utf-8')))
    print("EW list : {}\n".format(
        par.contents.parContractsString[1].value.decode('utf-8')))
...
def PrintPBNHand(title, remainCardsPBN):
    remainCards = [[[] for j in range(dds.DDS_SUITS)]
        ...
        if card:
            if first1 == 0:
                ...
                if handRelFirst == 0:
                    ...
                    hand = handRelFirst - 1
            remainCards[hand][suitInHand] = int(
                remainCards[hand][suitInHand]) | (hands.dbitMapRank[card] << 2)
